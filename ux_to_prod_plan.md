...
**✅ C2 — AppHeader במסכי auth (15.7):** Login, Register, VerifyEmail, SSOCountry, CompleteProfile. `AppHeader` קיבל `children` (תת-כותרת משתנה בתוך `.auth-header`); CSS: `.auth-page:not(.legal-page) > .app-head { margin: -24px -24px 0 }` — ההדר יושב ב-16px בעוד תוכן הטופס נשאר בריפוד 24px (`.legal-page` מוחרג כי הריפוד שלו 0). אומת חי בכל 5 המסכים (back 16 / logo 16, טופס x=24) + `/terms` לא נפגע. הוסרו כל שאריות `IconArrowRight` מחוץ ל-AppHeader. **✅ C3 — RecipeCard משותף ב-Feed+Recipes (15.7):** נמחקו הכרטיסים המקומיים + עותקי `authorName` הכפולים; שני העמודים משתמשים ב-`components/RecipeCard`. **תוקן סדר-דגלים בקומפוננט המשותף לקנוני** (ישראל מימין; היה הפוך, השפיע רק על Profile עד עכשיו). בונוס מהקומפוננט: dedupe דגל למשתמש ישראלי (🇮🇱 אחד במקום כפול). אומת חי: Feed + Recipes/קהילה זהים, דגלים 🇳🇿→🇮🇱 בסדר קנוני, build+console נקיים. `authorName` נשאר משוכפל רק ב-RecipePage — יטופל ב-B9 (UserIdentity). **📌 לב-B9: המשתמש ביקש לתקן יישור אנכי של הדגלים למרכז גובה השם + להציג לאישור.** **✅ B1 — IngredientEditor (15.7):** `src/components/IngredientEditor.jsx` controlled (`ingredients`+`onChange`); AddRecipe שלב 3 + EditRecipe מאמצים, נמחקו `setIng/addIng/delIng` הכפולים משני הקבצים. אומת חי (bypass זמני `?step=3`, הוסר): רינדור זהה + הוספה/הקלדה/מחיקה עובדים. **✅ B2 — StepEditor (15.7):** `src/components/StepEditor.jsx` controlled (`steps`+`onChange`); AddRecipe שלב 3 + EditRecipe מאמצים, נמחקו `setStepText/addStep/delStep` הכפולים. שלב חדש = `{text:'', duration_seconds:null}` (טיימרים בבעלות ה-AI parser בלבד, כמו קודם). אומת חי (bypass זמני `?step=3`, הוסר): רינדור זהה (מספור כחול, minHeight 44, resize none) + הוספה/הקלדה/מחיקה-עם-מספור-מחדש עובדים. **✅ B3 — TagInput (15.7):** `src/components/TagInput.jsx` — controlled על `tags` בלבד; טקסט הקלט + שליפת תגיות-הקהילה (allTags, public, לפי שכיחות) = state פנימי של הקומפוננט. נמחקו משני העמודים: fetch כפול, `addTag`, `tagSuggestions`, states `newTag`/`allTags`. אומת חי (bypass `?step=3`, הוסר): הצעות מנתוני אמת ("ב"→ביתי/ביצים), Enter מוסיף+מנקה, קליק-הצעה מוסיף, קליק-צ'יפ מסיר. **✅ B4 — VisibilityPicker (15.7):** `src/components/VisibilityPicker.jsx` controlled (`isPublic`+`onChange`); שני כרטיסים (שיתוף=גלובוס ירוק / אישי=מנעול סגול #a78bff). AddRecipe שלב 3 + EditRecipe מאמצים; נמחקו imports מתים `IconWorld`/`IconLock` (+`IconPlus`/`IconX` ב-AddRecipe שעברו לקומפוננטים). אומת חי (bypass `?step=3`, הוסר): פרטי כברירת-מחדל, בחירה מתחלפת נכון בין שני הכרטיסים. **✅ קבוצת עורכי Add/Edit (B1-B4) הושלמה במלואה.** **הבא: B5 (Drawer) — סשן חדש מומלץ (B5-B9).**
